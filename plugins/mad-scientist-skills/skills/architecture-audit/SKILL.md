...
| **Silent telemetry swallow** | Python (`except Exception: logger.warning(...)` or `except Exception: pass` in a hook, callback, observer, or fire-and-forget code path — e.g. a lifecycle hook that writes metrics to a Delta/Kafka/metrics-store). Grep for `except Exception:\s*\n\s*logger\.(warning|debug)` inside files matching `*hook*`, `*observer*`, `*callback*`, `*metric*`, `*telemetry*`, `*cost*`. | **Critical data integrity risk.** Observability failures hide indefinitely at WARNING level because standard error-log queries filter by level. Compounds into data-integrity bugs when no other signal surfaces the failure. Default exception handling in telemetry must be one of: (1) raise, (2) typed error return, (3) observable log at ERROR level. Never `logger.warning` in a fire-and-forget path. |
| **UDF empty-return on exception** | Python (`except Exception: return pd.DataFrame(columns=...)` or `return []` inside a closure passed to `applyInPandas`, `mapInPandas`, `map_partitions`, Dask `map_blocks`, Ray actor, or any distributed-executor API). Grep for `return\s+(pd\.DataFrame|\[\]|\{\})` inside `except` blocks within functions that are passed to `.applyInPandas(`, `.mapInPandas(`, `.map_partitions(`, `@ray.remote`, or similar. | **Critical — silent per-group data loss.** Executor-level failures vanish because the distributed framework concatenates UDF outputs without tracking which groups returned empty. Missing rows are invisible in aggregate output; downstream aggregates silently understate totals. Must propagate with group-key context: `raise RuntimeError(f"... failed for <key>={value}")`. |
| **Writer-to-target schema drift** | Python `StructType([StructField(...), ...])` or `pydantic.BaseModel` class definition in one file, `CREATE TABLE ... (...)` SQL DDL in another file, with no programmatic reconciliation test. Grep for any file containing `StructField(` paired with a `create_*.sql` or `CREATE TABLE` file in the same package without a `test_*schema*drift*` or `test_*schema*match*` test. | **Critical — fire-and-forget writers ship broken for weeks.** `DeltaTable.merge().whenMatchedUpdateAll()` validates target columns at parse time and raises `DELTA_MERGE_UNRESOLVED_EXPRESSION` when the source schema is missing a target column. Without a CI test that parses both schemas and asserts equality, a column-drop in one file and a corresponding edit missing in the other will silently fail every MERGE until somebody runs the downstream consumer manually. Must have a test that parses the DDL and asserts `set(ddl_cols) == set(code_schema.names)`. |
...
**Practitioner note on silent-swallow patterns**: a catch that logs at WARNING level inside a fire-and-forget code path is **structurally equivalent to no logging at all** because error-log queries filter by level. Audit requires checking not just *whether* the catch logs, but *at what level* and *through which observability channel* (alerting? metric? error-log query?). The 2026-04-12 `luxury-lakehouse` warm-tier blocker is a worked example: a `logger.warning(...)` in a 4-method cost-hook catch block hid a `DELTA_MERGE_UNRESOLVED_EXPRESSION` for 62+ hours until the dbt test that exercised the downstream table was wired into CI.
...
| **Writer/target schema reconciliation** | Is there a test that parses the target schema (CREATE TABLE DDL, OpenAPI spec, protobuf/avro schema, dbt contract) and asserts equality with the writer's in-code schema (Spark `StructType`, Pydantic `BaseModel`, Python dataclass)? A fire-and-forget writer with a drifted schema will silently fail every merge for weeks without the reconciliation test. For Delta MERGE specifically: `whenMatchedUpdateAll()` validates target columns at parse time and raises `DELTA_MERGE_UNRESOLVED_EXPRESSION` when source is missing a target column; without a reconciliation test, this failure hides behind any silent-swallow exception handler in the writer. | High |

This check is especially important for data platforms where dbt models publish schemas consumed by application-layer SQL queries — the dbt contract validates the write side, but the read-side SQL is often unvalidated raw strings. It is also critical for operational telemetry tables (cost tracking, run metrics, audit logs) where a dedicated writer (PySpark lifecycle hook, background daemon) MERGEs into a table whose DDL lives in a separate SQL file: code + DDL drift will silently break every write, potentially for weeks before detection.
...
| Schema as implicit contract | Data contracts between pipeline stages exist only as runtime DataFrame column access (string keys). No explicit schema definition (Pydantic model, dbt contract, protobuf/avro schema) at stage boundaries. **Also check for hardcoded `StructType` literals inside pipeline writer code paired with a separate `CREATE TABLE` DDL file with no automated reconciliation test** — a fire-and-forget writer with a drifted schema will silently fail every merge without the reconciliation test. See Phase 4 "Writer/target schema reconciliation" for the specific check. | Medium |
