...
| `except:` / `except Exception:` without logging | Python | Swallowed exception — silent failure. **Note**: catches WITH logging are NOT safe by default — see "Silent telemetry swallow" row below. A `logger.warning(...)` in a fire-and-forget path hides failures just as thoroughly as no logging at all. |
| **Silent telemetry swallow (`except Exception: logger.warning/debug` in hook/callback)** | Python — grep for `except\s+Exception[^:]*:\s*\n\s*(?:\w+\.)?logger\.(warning|debug)\(` inside files named `*hook*`, `*observer*`, `*callback*`, `*metric*`, `*telemetry*`, `*cost*`, or inside classes implementing `LifecycleHook`, `Observer`, etc. | **Critical.** Fire-and-forget pattern that "logs" the failure at WARNING or DEBUG level — invisible in standard error-log queries that filter by level. Must either log at ERROR or surface via a typed error-return path. The 2026-04-12 `luxury-lakehouse` warm-tier blocker is a worked example: 4 hook method catches at WARNING level hid a `DELTA_MERGE_UNRESOLVED_EXPRESSION` for 62+ hours. |
| **UDF empty-return on exception (inside `applyInPandas`/`mapInPandas`/distributed executor)** | Python — grep for `return\s+(pd\.DataFrame\(|\[\]|\{\}|empty)` inside `except` blocks within closures passed to `.applyInPandas(`, `.mapInPandas(`, `.map_partitions(`, `@ray.remote`, `dask.delayed`, `pandas_udf`. | **Critical.** Silent per-group data loss inside a distributed UDF. No single log line reports the drop; downstream aggregates silently include the missing data as if the group never existed. Must propagate with group-key context: `raise RuntimeError(f"... failed for <group_key>={value}") from exc`. |
| **Silent fallback to default value without observable signal** | Any — grep for `except\s+Exception[^:]*:\s*\n\s*\w+\s*=\s*(0\.5|None|False|[\{\[])` (defensive fallback assignment). | **High.** Silent model/value downgrade — pipeline produces degraded output without user-visible indicator. Must emit a metric, log at ERROR level, or mark the output with a fallback flag that consumers can inspect. |
| **Hook/callback registration without completion assertion test** | Python — any hook registered via `register_hook()`, `app.on_event()`, or similar, where no test exercises the hook and asserts the hook's output lands in the target sink (not just "the method was called"). | **High.** Telemetry is assumed to work based on registration, not on observed output. If the hook's write path silently breaks (schema drift, grants, connection), the registration still passes — failures only surface when a downstream consumer notices missing data. |
...
**Practitioner note on catches with logging**: a catch that logs at WARNING level inside a fire-and-forget code path is structurally equivalent to no logging at all — error-log queries filter by level, error dashboards don't surface warnings, and on-call alerting fires on ERROR not WARNING. Audit requires checking not just *whether* the catch logs, but *at what level* and *through which observability channel*. The default for telemetry / hook / callback exception handling is one of: (1) **raise** — preferred when the caller can handle it; (2) **typed error return** — return a structured value (dataclass, enum, sentinel) that the caller must destructure; (3) **ERROR-level log + typed error return** — only for fire-and-forget paths that must not crash the calling process. `logger.warning` and `logger.debug` in a fire-and-forget catch block are never correct.
